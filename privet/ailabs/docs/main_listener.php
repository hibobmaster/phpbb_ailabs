...
class main_listener implements EventSubscriberInterface
{
	...
	public function viewtopic($event)
	{
		...
		if ($s_quick_reply)
		{
			// https://www.phpbb.com/customise/db/extension/simple_mentions/faq/3696
	...
	{
		// Allow @mention on edit
		$handle = ['post', 'reply', 'quote', 'edit'];
		...
		// Allow @mention on edit
		$this->parse_message($event, $event['data']['message'], $event['data']['forum_id']);
	}
	...
	public function handle_post_approval($event)
	{
		...
		foreach ($data as $row)
		{
			...
			// Allow @mention on edit
			$this->parse_message($event, $row['post_text'], $row['forum_id'], false, $local_auth);
	 ...
	 */
// Allow @mention on edit
private function parse_message($event, $message, $forum_id, $current = true, auth $local_auth = null)
{
		if ($local_auth === null)
		{
		...
		if ($event['mode'] == 'edit' && !empty($event['data']['post_id'])) {
			$sql = 'SELECT user_id FROM ' . NOTIFICATIONS_TABLE . ' WHERE item_id = ' . (int) $event['data']['post_id'];
			$result = $this->db->sql_query($sql);
			while ($row = $this->db->sql_fetchrow($result))
				array_push($already_notified, $row['user_id']);
			$this->db->sql_freeresult($result);
		}
		...
		if (preg_match_all('#\[mention\]<\/s>(.*?)<e>\[\/mention\]#', $message, $matches, PREG_OFFSET_CAPTURE) !== 0)
		{
			...
			$this->db->sql_freeresult($result);
			// Allow @mention on edit
			$this->handle_matches($already_notified, $data, $forum_id, $current);
		}
	...
	// Allow @mention on edit
	private function handle_matches(array $already_notified, array $data, int $forum_id, $current)
	{
		$authCache = [];
		if (count($data))
		{
			foreach ($data as $index => $row)
			{
				...
				// Allow @mention on edit
				if (in_array($row['user_id'], $already_notified)) {
					continue;
				}
